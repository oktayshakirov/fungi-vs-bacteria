...
18. **Real art for the four variety enemy types** — Swarm / Shielded /
    Splitter / Healer were a tint and a scale on three shared bodies; each now
    carries an authored mesh that changes its silhouette, generated headlessly
    in Blender. Splitter and Healer no longer share a prefab. See Priority 2b
    below, and section 9 for the Blender pipeline.
...
| `EnemyArtSetup.BuildVariants` | Rebuilds the four variety enemy prefabs from base + trait | yes |
...
and a scale. Each now carries a **trait**: one small authored mesh parented
under the base body, generated by `Tools/Blender/enemy_traits.py` and fitted by
`EnemyArtSetup` (see section 4 for both).

| Type | Base body | Trait | Reads as |
|---|---|---|---|
| Swarm | FastEnemy | `CiliaFringe` | a skirt of pale cilia, i.e. motion |
| Shielded | ArmoredEnemy | `Carapace` | plates banded over the back |
| Splitter | BasicEnemy | `BudLobes` | amber daughter cells already bulging out |
| Healer | BasicEnemy | `SporeCrown` | a pale cap on a stalk, tallest of the four |

Why traits rather than four new bodies: the four base models are authored,
detailed and heavy (`ArmoredEnemy.obj` is 124k verts / 20MB), and they carry the
family look through shared BodySkin / EyeWhite / Iris / Pupile materials. A
hand-scripted full body would read as a different game, and four more 20MB OBJs
would add ~80MB to the repo. All four traits together are **28KB and under 560
triangles.**

Three things this pass settled, each only visible in a render:
- **A trait must not be tinted like a body.** The first version multiplied the
  biome tint into the trait's accent, and two of four vanished - purple lobes on
  a purple body, a green crown on a green body. Traits now opt out of
  `EnvironmentTheme.EnemyTint` entirely: the body still carries the biome, the
  trait is type identity and stays constant. Verified across environments 1, 3
  and 6.
- **Shielded and Armored were the same blue spiky sphere**, so the carapace was
  doing all the work of separating them. `ShieldedEnemy`'s body is now a dark
  slate blue, which also means the type is still identifiable **with its shield
  broken and the carapace hidden** - rendered as
  `close-ShieldedEnemy-shielddown.png`.
- **Splitter and Healer shared `BasicEnemy.prefab`**, so they shared a pool and
  a silhouette. They now have their own prefabs.

`BalanceSim` output is **byte-identical** before and after, which is the check
that this pass changed nothing but appearance.

What still needs a human: whether a trait is legible at phone size in a pack of
thirty, and whether the carapace disappearing reads as "shield broken" rather
than as a glitch.
...
- **Trait placement is expressed as FRACTIONS of the base body's measured
  bounds, never world units.** The four bases differ wildly (BasicEnemy spans
  6.4 model units at prefab scale 0.2; ArmoredEnemy spans 3.3 at scale 1), so
  `EnemyArtSetup` measures each body in the ROOT's local space and scales the
  trait from that. Two consequences: the width metric is the **mean** of the two
  horizontal extents, not the max (FastEnemy is 7.07 long against 2.27 wide, and
  the max sized Swarm's fringe against the body's length), and after changing a
  base model you must re-run `EnemyArtSetup.BuildVariants`.
- **An open surface's winding decides whether it exists.** The spore crown's cap
  was built as the obvious fan - `(apex, rim[s], rim[s+1])` - which points its
  normals DOWN, and URP culled it from every shot taken from above, which is
  every shot in this game. The crown rendered as six orbs around a bare stalk
  with no cap at all, and nothing warned about it. Same class of problem shaped
  the carapace: plates spanning the top hemisphere are a **lid** no matter how
  narrow their azimuth, so its latitude band starts well below the pole.
...
1. Distinct silhouettes per environment, so a biome changes shape and not only
   colour. The trait mechanism already supports this - it is a different trait
   per environment on the same body.
